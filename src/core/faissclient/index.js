...
var faissRPC = new proto.faiss.FaissService(
  "localhost:50052",
...
function initFaiss(nlist, nprobe, bpv, bpsv, vd, cbk) {
  ...
  utils.bulkGetVectors(db, (err, train_matrix) => {
    console.log(
      train_matrix.length,
      " documents retrieved for faiss index training"
    );
    if (!err) {
      var faiss_init_data = {
        nlist: nlist,
    ...
    initFaiss(
      ...
      (err, resp) => {
        if (!err) {
          ...
          sdb.put(
            { _id: "local_faissStatus", index_trained: true },
            (err, resp) => {
              if (!err) {
  ...
  if (!annoyIndexBuilt && !annoyIndexBuildProgress) {
    ...
    initFaiss(0, 0, 0, 0, init_config.vd, (err, resp) => {
      if (!err) {
        ...
        sdb.put(
          { _id: "local_annoyStatus", index_trained: true },
          (err, resp) => {
            if (!err) {
